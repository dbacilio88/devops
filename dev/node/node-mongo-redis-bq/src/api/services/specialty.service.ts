...
import { SpecialtyMapper } from "src/components/mappers/index.mapper";
...
import { SpecialtyDto } from "src/dtos";
import { v4 as uuidv4 } from 'uuid';
import { validate } from "class-validator";
import { SpecialtyRepository } from "../documents/contracts/Specialty.repository";

@Injectable()
export class SpecialtyService extends ServiceBase {

    constructor(
        readonly repository: SpecialtyRepository,
        readonly mapper: SpecialtyMapper
    ) {
    ...
    async create(dto: SpecialtyDto): Promise<any> {

        let query = dto.code;

        try {
            const result = await this.repository.findOne({ code: query })
                .then(async (res) => {
                    if (res) {
    ...
    async update(uuid: string, dto: SpecialtyDto): Promise<any> {
        try {
