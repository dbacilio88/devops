...
import { QuestionMapper } from "src/components/mappers/index.mapper";
...
import { QuestionDto } from "src/dtos";
import { v4 as uuidv4 } from 'uuid';
import { validate } from "class-validator";
import { QuestionRepository } from "../documents/contracts/Question.repository";
...
export class QuestionService extends ServiceBase {
    ...
    constructor(
        ...
        readonly mapper: QuestionMapper
    ) {
    ...
    async create(dto: QuestionDto): Promise<any> {

        let query = dto.question;

        try {
            const result = await this.repository.findOne({ question: query })
                .then(async (res) => {
                    if (res) {
    ...
    async update(uuid: string, dto: QuestionDto): Promise<any> {
        try {
