...
@Controller('Answer')
export class AnswerController extends ControllerBase {

    constructor(
        private readonly service: AnswerService,
        readonly businessResponse: BusinessResponse) {
        super("AnswerController", businessResponse)
    }
    ...
    @Post('save')
    async save(@Req() req, @Res() res, @Body() body: AnswerDto): Promise<any> {
        try {
    ...
    @Put('update/:uuid')
    async update(@Req() req, @Res() res, @Param('uuid') uuid: string, @Body() body: AnswerDto) {
        try {
