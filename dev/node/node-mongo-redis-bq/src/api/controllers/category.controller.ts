...
export class CategoryController extends ControllerBase {
    ...
    constructor(
        ...
        super("CategoryController", businessResponse)
    }
    ...
    @Post('save')
    async save(@Req() req, @Res() res, @Body() body: CategoryDto): Promise<any> {
        try {
    ...
    @Put('update/:uuid')
    async update(@Req() req, @Res() res, @Param('uuid') uuid: string, @Body() body: CategoryDto) {
        try {
